...
namespace YF.ECS.Db
{
    ...
    public class SCustomer : BaseEntity
    {
        ...
        public virtual bool BalanceWarn
        {
            ...
            {
                bool warn = false;
                MWS.BaseMetadata.CustomerType customerType = CustomerType.ToEnum<MWS.BaseMetadata.CustomerType>();
                if (customerType == MWS.BaseMetadata.CustomerType.Customer && MinBalanceAmount > BalanceAmount && MinBalanceAmount > 0)
                    warn = true;
                return warn;
            }
        }
        ...
        public virtual string RowStateName 
        {
            get 
            {
                return RowState.ToEnum<MWS.BaseMetadata.RowState>().ToDescription();
            }
        }
    }
}

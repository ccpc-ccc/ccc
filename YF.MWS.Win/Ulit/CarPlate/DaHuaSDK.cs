    ...
    public sealed class DHClient {
        ...
        public static bool DHGetNewDevConfig(int lLoginID, int lChannel, string strCommand, ref object obj, Type typeName, int nRuleSize, int waittime)
        {
            ...
            try
            {
                ...
                else if (strCommand.CompareTo(DHClient.CFG_CMD_TRAFFICGLOBAL) == 0)
                {
                    CFG_TRAFFICGLOBAL_INFO traffGobalInfo = new CFG_TRAFFICGLOBAL_INFO();
                    Marshal.StructureToPtr(traffGobalInfo, pOutBuf, true);
                }
        ...
        public static bool DHOperateTrafficList(Int32 lLoginID, ref NET_IN_OPERATE_TRAFFIC_LIST_RECORD pstInParam, ref NET_OUT_OPERATE_TRAFFIC_LIST_RECORD pstOutParam, int waittime)
        {
            bool bReturn = false;
            try
            {
                bReturn = CLIENT_OperateTrafficList(lLoginID, ref pstInParam, ref pstOutParam, waittime);
                VIDEOThrowLastError(bReturn);
            }
            catch (System.Exception ex)
            {
        ...
        // 按查询条件查询记录
        public static bool DHFindRecord(Int32 lLoginID, ref NET_IN_FIND_RECORD_PARAM stInParam, ref NET_OUT_FIND_RECORD_PARAM stOutParam, int waittime)
        {
            bool bReturn = false;
            try
            {
                bReturn = CLIENT_FindRecord(lLoginID, ref stInParam, ref stOutParam, waittime);
                VIDEOThrowLastError(bReturn);
            }
            catch (System.Exception ex)
            {
        ...
        public static UInt32 DHFindNextRecord(ref NET_IN_FIND_NEXT_RECORD_PARAM stInParam, ref NET_OUT_FIND_NEXT_RECORD_PARAM stOutParam, int waittime)
        {
            return CLIENT_FindNextRecord(ref stInParam, ref stOutParam, waittime);
        }

        public static bool DHFindRecordClose(Int32 lFindHandle)
        {
            return CLIENT_FindRecordClose(lFindHandle);
        }
        ...
        // 黑白名单操作
        [DllImport("dhnetsdk.dll")]
        private static extern bool CLIENT_OperateTrafficList(Int32 lLoginID, ref NET_IN_OPERATE_TRAFFIC_LIST_RECORD pstInParam, ref NET_OUT_OPERATE_TRAFFIC_LIST_RECORD pstOutParam, int waittime);

        [DllImport("dhnetsdk.dll")]
        // 按查询条件查询记录
        private static extern bool CLIENT_FindRecord(Int32 lLoginID, ref NET_IN_FIND_RECORD_PARAM stInParam, ref NET_OUT_FIND_RECORD_PARAM stOutParam, int waittime);

        [DllImport("dhnetsdk.dll")]
        // 查找记录:nFilecount:需要查询的条数, 返回值为媒体文件条数 返回值<nFilecount则相应时间段内的文件查询完毕
        private static extern UInt32 CLIENT_FindNextRecord(ref NET_IN_FIND_NEXT_RECORD_PARAM stInParam, ref NET_OUT_FIND_NEXT_RECORD_PARAM stOutParam, int waittime);

        [DllImport("dhnetsdk.dll")]
        // 结束记录查找
        private static extern bool CLIENT_FindRecordClose(Int32 lFindHandle);

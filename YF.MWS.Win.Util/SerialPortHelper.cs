...
namespace YF.MWS.Win.Util
{
    public class SerialPortHelper:IDisposable
    {
        ...
        /// 左指示灯状态切换线程
        /// </summary>
        //private Thread threadLightState;
        ...
        public delegate void ShowWeight(int deviceNo, double value);
        ...
        public SerialPortHelper(int deviceNo)
        {
            this.deviceNo = deviceNo;
            this.section = string.Format("Device{0}", this.deviceNo);
            this.deviceVersion = IniUtility.GetIniKeyValue(section, "Version", "XK3190-A6");
            this.deviceComFun = IniUtility.GetIniKeyValue(section, "ComFun", "0");
        ...
        public bool OpenPort()
        {
            try
            {
                if (this.serlPort != null && !this.serlPort.IsOpen)
                {
                    this.serlPort.Open();
                    isOpen = true;
                    Logger.Info("Port opened success");
                    //指示灯亮
                    if (this.OnStateChanged != null)
                        this.OnStateChanged(this.deviceNo, true);
                   this.serlPort.DataReceived += new SerialDataReceivedEventHandler(this.SerlPort_DataReceived);
